...
import { hangoutScene } from '../data/hangouts';
import { endingById } from '../data/endings';
import type { CharacterId, GlobalKey, StatKey, Line, Effect, Scene } from '../types';

export const routes = {...routesA,...routesB} as Record<CharacterId,Scene[]>;
export const ids=characters.map(c=>c.id);
export type Stats=Record<StatKey,number>;
export type GameState={version:1;name:string;seed:number;phase:'story'|'map'|'routeSelect'|'ending';segment:'common'|'route'|'hangout'|'harem';chapter:number;route:CharacterId|null;routeChapter:number;line:number;response:Line[]|null;stats:Record<CharacterId,Stats>;global:Record<GlobalKey,number>;flags:string[];visits:Record<CharacterId,number>;visitor:CharacterId|null;actions:number;visitedToday:CharacterId[];backlog:Line[];ending:string|null;haremOffered:boolean;haremChance:number;ngPlus:boolean;date:string;};
...
export function newGame(name:string,seed:number,ngPlus=false):GameState{
 ...
 return {version:1,name:name.trim(),seed:seed>>>0,phase:'story',segment:'common',chapter:0,route:null,routeChapter:0,line:0,response:null,stats:Object.fromEntries(ids.map((id,i)=>[id,{affection:10,trust:5,jealousy:0,special:specials[i]}])) as Record<CharacterId,Stats>,global:{harmony:40,fair:10,reputation:0,ethics:50,safety:50},flags:[],visits:Object.fromEntries(ids.map(id=>[id,0])) as Record<CharacterId,number>,visitor:null,actions:3,visitedToday:[],backlog:[],ending:null,haremOffered:false,haremChance:0,ngPlus,date:new Date().toISOString()};
}
export function activeScene(s:GameState):Scene{
 if(s.segment==='hangout'&&s.visitor)return {...hangoutScene(s.visitor,s.visits[s.visitor],s.chapter),day:commonScenes[s.chapter].day};
 if(s.segment==='route'&&s.route)return routes[s.route][s.routeChapter];
 if(s.segment==='harem')return haremScenes[s.routeChapter];
 return commonScenes[Math.min(s.chapter,commonScenes.length-1)];
}
...
export function finishScene(s:GameState,meta:Meta):GameState{
 ...
 if(s.segment==='hangout'){out.visits[s.visitor!]+=1;out.visitedToday.push(s.visitor!);out.actions-=1;out.visitor=null;out.segment='common';out.phase='map';return out;}
 if(s.segment==='common'){
...
export const nextDay=(s:GameState):GameState=>s.phase!=='map'||s.chapter>=commonScenes.length-1?s:({...s,phase:'story',segment:'common',chapter:s.chapter+1,line:0,response:null,visitor:null});
export function visit(s:GameState,id:CharacterId):GameState{if(s.phase!=='map'||s.actions<=0||s.visitedToday.includes(id))return s;return {...s,phase:'story',segment:'hangout',visitor:id,line:0,response:null};}
...
export function isGameState(s:unknown):s is GameState{
 ...
 if(v.version!==1||typeof v.name!=='string'||!validName(v.name)||!integer(v.seed,0,0xffffffff)
  ...
  ||!['story','map','routeSelect','ending'].includes(v.phase)||!['common','route','hangout','harem'].includes(v.segment)
