...
function roundsFor(id:CharacterId,key:string):ActivityRound[]{
 const n=hash(key);
 if(id==='world'){
  const symbols=['♪','●','▲','◆'];
  return [3,4,5].map((length,index)=>({
   ...
   symbols,pattern:Array.from({length},(_,step)=>(n+index*3+step*step+step)%symbols.length),action:'리프 듣기',
 ...
 if(id==='junyeon'){
  const plans:[string,string[],string[]][]=[
   ['깨진 기구 주변을 안전하게 정리할 순서를 함께 정한다.',['접근 막기','담당자 알리기','도구로 수거'],['접근 막기','담당자 알리기','도구로 수거']],
   ['라벨 없는 시료를 발견했을 때의 확인 순서를 맞춘다.',['사용 중지','기록 확인','담당자 확인'],['사용 중지','기록 확인','담당자 확인']],
   ['실패한 측정값을 다시 검증할 순서를 맞춘다.',['원값 보존','조건 기록','재측정'],['원값 보존','조건 기록','재측정']],
  ];
  return plans.map(([prompt,answer],index)=>({mode:'order' as const,prompt,items:rotate(answer,(n+index)%answer.length),answer,action:'순서 확정',explain:'준연이 남이 시킨 절차가 아니라 둘이 확인한 순서를 자기 목소리로 다시 읽었다.'}));
 }
 if(id==='hyunsol')return [28,54,76].map((base,index)=>{
  const target=Math.max(15,Math.min(85,base+((n>>index)%9)-4));
  return {mode:'balance' as const,prompt:['준연의 원본과 현솔의 수정본 사이에서 검증 강도를 맞춘다.','정확한 지적과 상대가 받아들일 수 있는 속도의 균형을 잡는다.','최종 발표본에서 확신과 보류 표시의 비율을 조정한다.'][index],target,tolerance:7,left:index===0?'원본 보존':'천천히 확인',right:index===0?'수정 반영':'즉시 수정',action:'이 값으로 검증',explain:'현솔은 정답 하나를 강요하지 않고, 네가 멈춘 지점을 함께 검토 기준으로 삼았다.'};
 });
 if(id==='taewoo')return [24,50,76].map((base,index)=>({
  ...
  target:base+((n>>index)%7)-3,tolerance:index===2?7:9,action:index===2?'마지막 박자 잡기':'지금 스텝!',
 ...
 if(id==='taehun')return [31,58,79].map((base,index)=>({
  ...
  target:base+((n>>index)%7)-3,tolerance:index===2?6:8,action:'관측 기록',
 ...
 return [34,57,73].map((base,index)=>{
  ...
  return {mode:'balance' as const,prompt:['서율의 밑그림이 묻히지 않도록 배경의 색 농도를 맞춘다.','밴드 편곡에서 건반과 보컬 사이의 빈 공간을 조절한다.','완성할 부분과 일부러 남겨 둘 여백의 비율을 고른다.'][index],target,tolerance:8,left:index===1?'건반':'여백',right:index===1?'보컬':'채우기',action:'이 균형으로 남기기',explain:index===2?'서율은 네가 전부 채우지 않은 여백에 작은 서명을 하나 더 남겼다.':'같은 색과 소리도 둘이 정한 비율에서 새로운 장면이 되었다.'};
 });
}
...
export function activityFor(id:CharacterId,location:LocationId,seed:number,chapter:number,visit:number):Activity{
 ...
 return {
  ...
  rounds:roundsFor(id,`${seed}:${chapter}:${visit}:${location}`),
 };
}
