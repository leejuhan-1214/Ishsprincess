import {blankMeta,restoreGame,type GameState,type Meta} from './game';
...
export function getSave(slot:string):Save|null{const v=readJSON('save:'+slot) as Save|null;if(!v||typeof v.savedAt!=='string')return null;const state=restoreGame(v.state);return state?{...v,state}:null;}
