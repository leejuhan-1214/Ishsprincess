...
function StoryPortrait({id,sceneKey,text,stats,className=''}:{id:CharacterId;sceneKey:string;text:string;stats:RelationshipStats;className?:string}){
 const expression=expressionFor(id,text,sceneKey,stats);
 return <div role="img" aria-label={`${characterById[id].name} · ${expression.name} 표정`} className={`portrait actor-portrait story-expression ${className}`} data-expression={expression.name} style={{backgroundImage:`url(${asset(expression.asset)})`}}/>;
}
...
export default function App(){
 ...
 const choiceActor=choiceVisible?[...lines].reverse().find(item=>ids.includes(item.speaker as CharacterId))?.speaker as CharacterId|undefined:undefined;
 const stagedActor=actor??choiceActor;
 const stagedText=lineText||lines.at(-1)?.text||'';
 ...
 return <main className={`app ${game?'in-game':'on-title'}`}>
  ...
  {game.phase==='story'&&<section className="story-screen"><div className="scene-meta"><span className="eyebrow">{game.segment==='common'?'COMMON ROUTE':game.segment==='hangout'?(scene!.id.includes('-event-')?'SURPRISE EVENT':'AFTER SCHOOL'):game.segment==='harem'?'HIDDEN ROUTE':`${characterById[game.route!].name} ROUTE`}</span><h2>{scene!.title}</h2><span><MapPin size={13}/>{locationById[scene!.location].name}</span></div><div className="chapter-ribbon">{game.segment==='common'?String(game.chapter).padStart(2,'0'):String(game.routeChapter+1).padStart(2,'0')}<span>CHAPTER</span></div>{stagedActor&&<div className="actor-panel" key={`${stagedActor}:${scene!.id}:${game.line}`}><StoryPortrait id={stagedActor} sceneKey={`${scene!.id}:${game.line}`} text={stagedText} stats={game.stats[stagedActor]}/><div className="actor-caption"><span style={{background:characterById[stagedActor].color}}/>{characterById[stagedActor].role}<small>{expressionFor(stagedActor,stagedText,`${scene!.id}:${game.line}`,game.stats[stagedActor]).name}</small></div></div>}
   {choiceVisible?<div className="choice-block"><div className="choice-prompt"><span/><Sparkles size={15}/>{decisionPrompts[scene!.id]??(game.segment==='hangout'?scene!.title:`${scene!.title} · 지금의 대답`)}<span/></div>{scene!.choices.map((c,i)=><button key={c.id} onClick={()=>pick(i)}><small className={c.label?'choice-tag':undefined}>{c.label??String(i+1).padStart(2,'0')}</small><span>{formatted(c.text)}</span><ChevronRight size={19}/></button>)}</div>:<button className={`dialogue ${line?.speaker==='narrator'?'narrator-dialogue':''}`} onClick={next} aria-label="다음 대사">{line?.speaker!=='narrator'&&<div className="speaker-line"><span style={{borderColor:actor?characterById[actor].color:undefined}}>{line?label(line.speaker,game.name):'이어서'}</span></div>}<p>{lineText.slice(0,visible)}<span className="type-cursor">{visible<lineText.length?'▏':''}</span></p><div className="dialogue-bottom"><span>{visible<lineText.length?'CLICK TO REVEAL':'CLICK OR PRESS SPACE'}</span><ChevronRight size={17}/></div></button>}
